...
router.post("/", requireAuth, (req, res) => {
  ...
  db.prepare(`INSERT INTO ratings (id, request_id, rater_id, ratee_id, score, comment) VALUES (?, ?, ?, ?, ?, ?)`).run(
    uuid(),
    request_id,
    req.user.id,
    ratee_id,
  ...
  const agg = db.prepare("SELECT AVG(score) as avg, COUNT(*) as count FROM ratings WHERE ratee_id = ?").get(ratee_id);
  db.prepare("UPDATE users SET rating_avg = ?, rating_count = ? WHERE id = ?").run(agg.avg, agg.count, ratee_id);
